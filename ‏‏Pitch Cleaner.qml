...
MuseScore {
	...
	function mergeTiedNotes(notes, staffIdx) {
		...
		for (var i = 0; i < notes.length; i++) {
			...
			if (!elem || !elem.tieBack && !elem.tieForward) {
				continue;
			}
	...
	function getParent(tp, e) {
		...
		return e && e.type === Element[tp] ? e : null;
	}
	function optimizeNotes() {
		...
		for( var staffIdx = startStaff; staffIdx < endStaff; staffIdx++ ){
			updateStaff(staffIdx, startSeg, endSeg, tickTolerance);
		}
		...
		msg('Finished. Removed ' + removedCount + ' notes.');
	}
	...
	function updateStaff(staffIdx, startSeg, endSeg, tickTolerance) {
